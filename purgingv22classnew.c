/* purgingv22class.c (viabilidad 1-sod1 1 1-sod2) */
...
double d_a, alfa_a, b, numAASD, numAaSD, numaaSD;
...
double L, PS, Lambda_s, Lambda_L, Lambda_OD, sod1, sod2;
...
struct acc del[NN], let[NN], od[NN], qdel[NN], sdel[NN], hsdel[NN], qlet[NN], qod[NN];
...
struct acc qod_000_005[NN], qod_005_015[NN], qod_015_025[NN], qod_025_035[NN], qod_035_045[NN], qod_045_055[NN], qod_055_065[NN], qod_065_075[NN], qod_075_085[NN], qod_085_095[NN], qod_095_100[NN];
	...
	if (initialgen[k][l] != (-99))
	{
		...
		if ((q[k][l] > 0.0) && (q[k][l] < 1.0))
		{
			...
			else if (hs[k][l]==99)
			{
				accum (&od[gen], 1.0);
				accum (&qod[gen], q[k][l]);
				if ((q[k][l] > 0.0) && (q[k][l] < 0.05))		accum (&qod_000_005[gen], 1.0);
				else if ((q[k][l] > 0.05) && (q[k][l] < 0.15))	accum (&qod_005_015[gen], 1.0);
				else if ((q[k][l] > 0.15) && (q[k][l] < 0.25))	accum (&qod_015_025[gen], 1.0);
				else if ((q[k][l] > 0.25) && (q[k][l] < 0.35))	accum (&qod_025_035[gen], 1.0);
				else if ((q[k][l] > 0.35) && (q[k][l] < 0.45))	accum (&qod_035_045[gen], 1.0);
				else if ((q[k][l] > 0.45) && (q[k][l] < 0.55))	accum (&qod_045_055[gen], 1.0);
				else if ((q[k][l] > 0.55) && (q[k][l] < 0.65))	accum (&qod_055_065[gen], 1.0);
				else if ((q[k][l] > 0.65) && (q[k][l] < 0.75))	accum (&qod_065_075[gen], 1.0);
				else if ((q[k][l] > 0.75) && (q[k][l] < 0.85))	accum (&qod_075_085[gen], 1.0);
				else if ((q[k][l] > 0.85) && (q[k][l] < 0.95))	accum (&qod_085_095[gen], 1.0);
				else if ((q[k][l] > 0.95) && (q[k][l] < 1.0))	accum (&qod_095_100[gen], 1.0);
			}
			else
			{
				accum (&del[gen], 1.0);
				accum (&qdel[gen], q[k][l]);
				accum (&sdel[gen], s[k][l]);
				accum (&hsdel[gen], hs[k][l]);
			}
		}
...
void genotypic_values (int gm[][MM][2], double s[][31], double hs[][31], double genvalm_s[], int RM[])
{
	...
	for (i=0; i<NIND; i++)
	{
		...
		if ((initialgen[k][l] != (-99)) && (hs[k][l] == 99))
		{
	    		if (((gm[i][k][0] & RM[l])==RM[l])&&((gm[i][k][1] & RM[l])==RM[l]))
			{
				genvalm_s[i] *= (1.0 - sod2);
				numaaSD ++;
			}
			else if (((gm[i][k][0] & RM[l])!=RM[l])&&((gm[i][k][1] & RM[l])!=RM[l]))
			{
				genvalm_s[i] *= (1.0 - sod1);
				numAASD ++;
			}
			else	
			{
		...
		//if (tracelevel!=0)	if (gen == 1) fprintf(fptr,"i = %d   addedfixed_s = %f  sod1 = %f  sod2 = %f  numAASD = %f  numAaSD = %f  numaaSD = %f\n", i, addedfixed_s, sod1, sod2, numAASD, numAaSD, numaaSD);
	}
	...
	//if (tracelevel!=0) if (gen == 1)   fprintf(fptr,"gen=%d va_del=%f va_let=%f va_od=%f\n", gen, va_del, va_let, va_od);
}
...
generation1 ()
{
	...
	for (i=0; i<NIND; i++)
	{
		...
		if (tracelevel!=0)	if(pm_sG0[i] == 0.0) fprintf(fptr,"gen = %d    i = %d    numAASD = %f  numAaSD = %f  numaaSD = %f  pm_sG0 = %f\n", gen, i, numAASD, numAaSD, numaaSD, pm_sG0[i]);
	}
	...
	for (i=0; i<((NIND/2)*J); i++)
	{
		...
		//if (tracelevel!=0)   if (gen == 1)  fprintf (fptr,"%d\t%d\n", p1, p2);
		...
		if (tracelevel!=0)	if(pm_sG1[i] == 0.0)  fprintf(fptr,"gen = %d    i = %d   numAASD = %f  numAaSD = %f  numaaSD = %f  pm_sG1 = %f\n", gen, i, numAASD, numAaSD, numaaSD, pm_sG1[i]);
...
//	LOG SCALING
	...
	//if (tracelevel!=0)	if (gen == 1)   for (i=0; i<NIND; i+=2)	fprintf (fptr,"mean_parents_G0[%d] = %f    mean_family_G1[%d] = %f\n", i, mean_parents_G0[i], i, mean_family_G1[i]);

	double sum_parents = 0.0;
	double sum_parents2 = 0.0;
	...
	for (i=0; i<NIND; i+=2)
	{
		...
		sum_parents2 += mean_parents_G0[i] * mean_parents_G0[i];
	...
	var_G0 = (sum_parents2 - (sum_parents * sum_parents)/I) / (I - 1.0);
	//if (tracelevel!=0)	if (gen == 1)   fprintf (fptr,"sum_parents = %f     sum_offspring = %f      sum_parents_offspring = %f     VA_G0 = %f     var_G0 = %f     h2 = %f\n", sum_parents, sum_offspring, sum_parents_offspring, 2.0*VA_G0, var_G0, VA_G0/var_G0);
...
//	LOG SCALING
	...
	if (pm_sG1[i] != 0.0)
	{
		Xij2 += (log(pm_sG1[i]) * log(pm_sG1[i]));
		Xoo += log(pm_sG1[i]);
	}
	...
	if (tracelevel!=0)	fprintf (fptr,"gen= %d  MSF = %f  MSE = %f  vf = %f    VD = %f\n", gen, MSF, MSE, vf, 4.0*(vf - VA_G0));
...
printout()
{
	...
	fprintf(fgen,"\ngen	n.del	   sdel	   hsdel	 qdel	n.let	 qlet	 n.od	 qod     fixsd	  lostsd\n");
	for (gen=0; gen<=generations; gen++)
	{
        fprintf(fgen, "%d    %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f\n",
		gen, accsum(&del[gen])/(replicates*lines), accmean(&sdel[gen]), accmean(&hsdel[gen]), accmean(&qdel[gen]), accsum(&let[gen])/(replicates*lines), accmean(&qlet[gen]), accsum(&od[gen])/(replicates*lines), accmean(&qod[gen]), accsum(&fix_sd[gen])/(replicates*lines), accsum(&lost_sd[gen])/(replicates*lines));
	}
	...
	fprintf(fgen,"\ngenqod0.0   qod0.1   qod0.2   qod0.3   qod0.4   qod0.5   qod0.6   qod0.7   qod0.8   qod0.9   qod1.0\n");
	for (gen=0; gen<=generations; gen++)
	{
        fprintf(fgen, "%d    %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f\n",
		gen, accsum(&qod_000_005[gen])/(replicates*lines), accsum(&qod_005_015[gen])/(replicates*lines), accsum(&qod_015_025[gen])/(replicates*lines),
		accsum(&qod_025_035[gen])/(replicates*lines), accsum(&qod_035_045[gen])/(replicates*lines), accsum(&qod_045_055[gen])/(replicates*lines),
		accsum(&qod_055_065[gen])/(replicates*lines), accsum(&qod_065_075[gen])/(replicates*lines), accsum(&qod_075_085[gen])/(replicates*lines),
		accsum(&qod_085_095[gen])/(replicates*lines), accsum(&qod_095_100[gen])/(replicates*lines));
	}
