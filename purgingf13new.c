...
getinputs()
{
	...
	getrealandskip("Lambda_OD (s=0.02,h=1.5):",&Lambda_OD,0.0,(double)infinity);
...
headings()
{
	...
	fprintf(fgen,"\nN=%d  PS=%f N.S-LOCI=%d    N.N-LOCI=%d    lines=%d   reps=%d   gens=%d\n    L=%f   Lambda_s=%6.4f   Lambda_L=%6.4f   Lambda_OD=%6.5f\n    ave_s=%6.4f   beta_s=%6.4f   alpha_s=%6.4f  k_s=%6.4f   ave_hs=%6.4f\n", NIND, PS, TOTLOCI-NCRO, NCRO, lines, replicates, generations, L, Lambda_s, Lambda_L, Lambda_OD, ave_s, beta_s, alpha_s, k_s, ave_hs);
...
//		if (tracelevel!=0)	fprintf(fptr,"i = %d   addedfixed_s = %f  sod = %f  numAaSD = %f\n", i, addedfixed_s, sod, numAaSD);
	}
...
//		if (tracelevel!=0)	fprintf(fptr," %d    sod = %f  numAaSD = %f  pm_sG2 = %f\n", i, sod, numAaSD, pm_sG2[i]);
