...
/* naturalv14class.c (modelo clásico de viabilidad con sobredominancia 1-sod1 1 1-sod2 sin addedfixed for SD) */
...
int muts, mutsL, mutsOD, marker, whatproportion;
int lastinmutantspoissontable, lastinmutantspoissontableL, lastinmutantspoissontableOD, lastinrecpoissontable;
...
double Lambda_a, Lambda_L, Lambda_OD, sod1, sod2, Psi, threshold, h2a;
double PS, L, AA, Aa, aa, q[MMM][31], numAaSD;
double meanmuts_a[NN] , meanmuts_s[NN] , meanmutsL_s[NN] , meanmutsL_sgamma[NN];
...
headings()
{
	...
	fprintf(fgen,"\n\n N=%d    PS=%4.2f    L=%f   N.S-LOCI=%d    N.N-LOCI=%d    gens=%d\n Lambda_a=%6.4f   Lambda_L=%6.4f   Lambda_OD=%6.4f   sod1=%6.4f   sod2=%6.4f   ave_aL=%6.4f    Psi=%6.4f   threshold=%6.4f   rho=%6.4f   Vs=%6.4f \n ave_s=%6.4f   PP_s=%6.4f   beta_s=%6.4f   alpha_s=%6.4f  dom=%d   k_s=%6.4f   ave_hs=%6.4f\n ave_a=%6.4f   PP_a=%6.4f   beta_a=%6.4f   alpha_a=%6.4f   k_a=%6.4f   ave_ha=%6.4f\n", NIND, PS, L, TOTLOCI-NCRO, NCRO, generations, Lambda_a, Lambda_L, Lambda_OD, sod1, sod2, ave_aL, Psi, threshold, rho, Vs, ave_s, P_s, beta_s, alpha_s, dom, k_s, ave_hs, ave_a, P_a, beta_a, alpha_a, k_a, ave_ha);
    	fclose(fgen);
}
...
double genvalm_a[], a[][31], ha[][31];
{
	...
	for (i=0; i<NIND; i++)
	{
		...
		else if ((initialgen[k][l] != (-99)) && (hs[k][l] == 99))
		{
	           if (((gm[i][k][0] & RM[l])==RM[l])&&((gm[i][k][1] & RM[l])==RM[l]))
               {
...
double pm_s[];
{
	...
	double family_sum2=0.0, sq_pm_a, rnd, b;
	int numAaSD;
...
//if (tracelevel!=0)  fprintf (fptr," %d    genvalm_a = %f      genvalm_s = %f\n", i, genvalm_a[i], genvalm_s[i]);
		...
		if (pm_s[i] > 1.0) accum (&pm_m1[gen/block], 1.0);
...
double q[][31], s[][31], hs[][31], a[][31], ha[][31], cumH[][31];
{
    ...
    if (tracelevel!=0)    fprintf(fptr,"\n New selected mutants muts=%d mutsL=%d mutOD=%d\n", muts, mutsL, mutsOD);
    ...
    if (countNoSS != 0)
    {
		...
		for (m=0; m<countNoSS; m++)
		{
			...
			/* ****** Lethal and OD mutations with effect on the trait ****** */
				...
				if(whatproportion==0)
				{
					if(uniform() < Psi)
					{
						...
						else if(rho==99.0)
						{
							...
							{
								za=1.0;
								meanmutsL_sgamma[(gen+1)/block] +=  1.0;
							}
							zs = za;
							if (tracelevel!=0)    fprintf(fptr,"\n MUTATION Psi=1 RHO=99     zs=%f\n", zs);
						}
						else
						{
						...
						//if (tracelevel!=0)    fprintf(fptr,"\n MUTATION ASSIGNED 1     zs=%f     s=%f\n", zs, s[NoSS_k[m]][NoSS_l[m]]);
...
printout()
{
	...
	fprintf(fgen, "         pm_m1=%6.4f        gmean_s=%6.4f+-%6.4f     gvar_s=%6.4f+-%6.4f    C2=%6.4f+-%6.4f \n         gmean_a=%6.4f+-%6.4f     gvar_a=%6.4f+-%6.4f    H2_a=%6.4f \n", accsum(&pm_m1[gen/block]), accmean(&gmean_s[gen/block]), se(&gmean_s[gen/block]), accmean(&gvar_s[gen/block]), se(&gvar_s[gen/block]), accmean(&C2[gen/block]), se(&C2[gen/block]), accmean(&gmean_a[gen/block]), se(&gmean_a[gen/block]), accmean(&gvar_a[gen/block]), se(&gvar_a[gen/block]), accmean(&gvar_a[gen/block]) / (accmean(&gvar_a[gen/block]) + VE));

	fprintf(fgen, "         VA_s=%6.4f+-%6.4f      VD_s=%6.4f+-%6.4f      B_s=%6.4f\n", accmean(&VA_s[gen/block]), se(&VA_s[gen/block]), accmean(&VD_s[gen/block]), se(&VD_s[gen/block]), sqrt(accmean(&VD_s[gen/block])));

	fprintf(fgen, "         VA_sd=%6.4f+-%6.4f      VD_sd=%6.4f+-%6.4f      B_sd=%6.4f\n", accmean(&VA_sd[gen/block]), se(&VA_sd[gen/block]), accmean(&VD_sd[gen/block]), se(&VD_sd[gen/block]), sqrt(accmean(&VD_sd[gen/block])));

	fprintf(fgen, "         addedfixed_s=%6.4f      Wmax=%6.4f\n", addedfixed_s, accmean(&Wmax[gen/block]));
	fprintf(fgen, "         L_s=%6.4f\n", (accmean(&Wmax[gen/block]) - accmean(&gmean_s[gen/block])) / accmean(&Wmax[gen/block]));
    ...
    fprintf (fgen, "         Ave. number of lethal mutants gamma : %f\n", 	(double)meanmutsL_sgamma[gen/block]/(double)block);
