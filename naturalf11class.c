...
/* naturalf11class.c (modelo de fecundidad con sobredominancia  1-sod1 - 1 - 1-sod2)*/
...
int muts, mutsL, mutsQN, mutsOD, marker, whatproportion;
int lastinmutantspoissontable, lastinmutantspoissontableL, lastinmutantspoissontableQN, lastinmutantspoissontableOD, lastinrecpoissontable;
int NIND, NCRO, NLOCI, TOTLOCI, block, model_selection;
int gen, generations, i, j, k, l, m;
...
double Lambda_a, Lambda_L, Lambda_QN, Lambda_OD, Psi, threshold, h2a, numAASD, numAaSD, numaaSD;
...
double mutantspoissontable[maxmpt], mutantspoissontableL[maxmpt], mutantspoissontableQN[maxmpt], mutantspoissontableOD[maxmpt];
double freqn[NN], CHn[NN], fixn[NN], lostn[NN], othern[NN];
double freq[NN], CH[NN], fix[NN], lost[NN], other[NN];
double fix_sd[NN], lost_sd[NN], other_sd[NN];
...
struct acc ng_all, a_all, s_all, ha_all, hs_all, q_all, VA_all, VD_all;
struct acc Ave_ng_all, Ave_a_all, Ave_s_all, Ave_ha_all, Ave_hs_all, Ave_q_all, Ave_VA_all, Ave_VD_all;

struct acc ng_od, a_od, s_od, ha_od, hs_od, q_od, VA_od, VD_od;
struct acc Ave_ng_od, Ave_a_od, Ave_s_od, Ave_ha_od, Ave_hs_od, Ave_q_od, Ave_VA_od, Ave_VD_od;
struct acc qod_000_005, qod_005_015, qod_015_025, qod_025_035, qod_035_045, qod_045_055, qod_055_065, qod_065_075, qod_075_085, qod_085_095, qod_095_100;
struct acc Ave_qod_000_005, Ave_qod_005_015, Ave_qod_015_025, Ave_qod_025_035, Ave_qod_035_045, Ave_qod_045_055, Ave_qod_055_065, Ave_qod_065_075, Ave_qod_075_085, Ave_qod_085_095, Ave_qod_095_100;
...
struct acc VA_s[NN], VD_s[NN], Wmax[NN];
struct acc VA_sd[NN], VD_sd[NN];
struct acc numberofrecom[NN];
...
getinputs()
{
	...
	getrealandskip("Lambda_QN (s=0.005,h=0.5):",&Lambda_QN,0.0,(double)infinity);
...
headings()
{
	...
	fprintf(fgen,"\n\n N=%d    PS=%4.2f    L=%f   N.S-LOCI=%d    N.N-LOCI=%d    gens=%d\n Lambda_a=%6.4f   Lambda_L=%6.4f   Lambda_QN=%6.4f   Lambda_OD=%6.4f   sod1=%6.4f   sod2=%6.4f   ave_aL=%6.4f    Psi=%6.4f   threshold=%6.4f   rho=%6.4f   Vs=%6.4f \n ave_s=%6.4f   PP_s=%6.4f   beta_s=%6.4f   alpha_s=%6.4f  dom=%d   k_s=%6.4f   ave_hs=%6.4f\n ave_a=%6.4f   PP_a=%6.4f   beta_a=%6.4f   alpha_a=%6.4f   k_a=%6.4f   ave_ha=%6.4f\n", NIND, PS, L, TOTLOCI-NCRO, NCRO, generations, Lambda_a, Lambda_L, Lambda_QN, Lambda_OD, sod1, sod2, ave_aL, Psi, threshold, rho, Vs, ave_s, P_s, beta_s, alpha_s, dom, k_s, ave_hs, ave_a, P_a, beta_a, alpha_a, k_a, ave_ha);
    	fclose(fgen);
}
...
int gm[][MMM][2], initialgen[][31];
{
    ...
    if ( (exp(-2.0*(double)NIND*Lambda_QN) != 0.0)&&
			(2.0*(double)NIND*Lambda_QN < normalthreshold) )
    generatepoissontable(2.0*(double)NIND*Lambda_QN, &lastinmutantspoissontableQN, mutantspoissontableQN, maxmpt-1);
    
    if ( (exp(-2.0*(double)NIND*Lambda_OD) != 0.0)&&
			(2.0*(double)NIND*Lambda_OD < normalthreshold) )
	...
	for (i=0; i<NIND; i++)
	{
		...
		if (initialgen[k][l] != (-99))
		{
		  ...
		  else if (hs[k][l] == 99)
		  {
				...
				{
					numaaSD ++;
					if (model_selection==1)    genvalm_s[i] *= (1.0 - sod2);
					else    genvalm_s[i] += (-sod2);
					genvalm_a[i] += (-sod2);
					//if (tracelevel!=0)	fprintf(fptr,"gen = %d   i = %d   k = %d   l = %d   HOMO aa\n", gen, i, k, l);
				}
				else if (((gm[i][k][0] & RM[l])!=RM[l])&&((gm[i][k][1] & RM[l])!=RM[l]))
				{
					numAASD ++;
					if (model_selection==1)    genvalm_s[i] *= (1.0 - sod1);
					else    genvalm_s[i] += (-sod1);
					genvalm_a[i] += (-sod1);
					//if (tracelevel!=0)	fprintf(fptr,"gen = %d   i = %d   k = %d   l = %d   HOMO AA\n", gen, i, k, l);
				}
				else
				{
					numAaSD ++;
				 	if (model_selection==1)     genvalm_s[i] *= 1.0;
					else    genvalm_s[i] += 0.0;
					genvalm_a[i] += 0.0;
					//if (tracelevel!=0)	fprintf(fptr,"gen = %d   i = %d   k = %d   l = %d   HET Aa\n", gen, i, k, l);
				}
		  }
		}
        
		//if (tracelevel!=0)	fprintf(fptr,"gen = %d   i = %d   addedfixed_s = %f  numAASD = %f   numAaSD = %f   numaaSD = %f   genvalm_s = %f\n", gen, i, addedfixed_s, numAASD, numAaSD, numaaSD, genvalm_s[i]);
	...
	for (i=0; i<NIND; i++)
	{
		...
		*/
		...
		{	    /* ************** Restricted recombination ***************** */
			...
			accum(&numberofrecom[gen/block], numberrecs);
...
double q[][31], s[][31], hs[][31], a[][31], ha[][31], cumH[][31];
{
 	...
 	for (l=1; l<NLOCI; l++)
	{
	    ...
	    else if (q[k][l]==0.0)
	    {
			...
			{
				lost_sd[(gen+1)/block] += 1.0;
				if (tracelevel!=0) fprintf(fptr,"LOST_OD gen=%d\t%d\t%d\t%1.0f\t%1.0f\t%1.0f\tq=%f\tinigen=%d\taddedfixed=%f\n",gen,k,l,AA,Aa,aa,q[k][l],initialgen[k][l], addedfixed_s);
			}
	    ...
	    else if (q[k][l]==1.0)
	    {
			...
			{
				fix_sd[(gen+1)/block] += 1.0;
				if (tracelevel!=0) fprintf(fptr,"FIX_OD gen=%d\t%d\t%d\t%1.0f\t%1.0f\t%1.0f\tq=%f\tinigen=%d\taddedfixed=%f\n",gen,k,l,AA,Aa,aa,q[k][l],initialgen[k][l], addedfixed_s);
			}
	    ...
	    else
	    {
		  ...
		  if (hs[k][l] != 99)
		  {
			...
			d_a = (1.0 + (a[k][l]*ha[k][l])) - ((2.0 + a[k][l]) / 2.0);
			alfa_a = (-a[k][l]/2.0) + ( d_a * (2.0*q[k][l] - 1.0) );

			d_s = (1.0 + (s[k][l]*hs[k][l])) - ((2.0 + s[k][l]) / 2.0);
			alfa_s = (-s[k][l]/2.0) + ( d_s * (2.0*q[k][l] - 1.0) );

			VAs += 2.0 * alfa_s * alfa_s * q[k][l] * (1.0 - q[k][l]);
			VDs += (2.0 * d_s * q[k][l] * (1.0-q[k][l])) * (2.0 * d_s * q[k][l] * (1.0-q[k][l]));
		  }
		  else if (hs[k][l] == 99)
		  {
			numSegSD ++;
            
			other_sd[(gen+1)/block] += 1.0;

			alfa_a = (q[k][l]*sod2) - ((1.0-q[k][l])*sod1);
			alfa_s = (q[k][l]*sod2) - ((1.0-q[k][l])*sod1);

			d_a = (sod1 + sod2) / 2.0;
			d_s = (sod1 + sod2) / 2.0;

			VAsd += 2.0 * alfa_s * alfa_s * q[k][l] * (1.0 - q[k][l]);
			VDsd += (2.0 * d_s * q[k][l] * (1.0-q[k][l])) * (2.0 * d_s * q[k][l] * (1.0-q[k][l]));

			if (tracelevel!=0) fprintf(fptr,"SEG_OD gen=%d\t%d\t%d\t%1.0f\t%1.0f\t%1.0f\tq=%f\tinigen=%d\taddedfixed=%f\n",gen,k,l,AA,Aa,aa,q[k][l],initialgen[k][l], addedfixed_s);
		  }
    ...
    accum (&VA_sd[(gen+1)/block], VAsd);
    accum (&VD_sd[(gen+1)/block], VDsd);
    ...
    accum (&Wmax[gen/block], addedfixed_s);
}
...
distribution_gens_on_q()
{
  ...
  accum (&q_all,  q[k][l]);
	...
	if (hs[k][l]==99)
	{
		...
		accum (&s_od,  s[k][l]);
		accum (&q_od,  q[k][l]);
		if (a[k][l] < 0.0)	accum (&a_od,  (-a[k][l]));
		else		accum (&a_od,  a[k][l]);
		if (s[k][l] != 0.0)   accum (&hs_od,  hs[k][l]);
		accum (&ha_od,  ha[k][l]);
		accum (&VA_od,  2.0 * alfa_a * alfa_a * q[k][l] * (1.0 - q[k][l]));
		accum (&VD_od,  pow(2.0 * d_a * q[k][l] * (1.0 - q[k][l]), 2.0));

		if ((q[k][l] > 0.0) && (q[k][l] < 0.05))		accum (&qod_000_005, 1.0);
		else if ((q[k][l] > 0.05) && (q[k][l] < 0.15))	accum (&qod_005_015, 1.0);
		else if ((q[k][l] > 0.15) && (q[k][l] < 0.25))	accum (&qod_015_025, 1.0);
		else if ((q[k][l] > 0.25) && (q[k][l] < 0.35))	accum (&qod_025_035, 1.0);
		else if ((q[k][l] > 0.35) && (q[k][l] < 0.45))	accum (&qod_035_045, 1.0);
		else if ((q[k][l] > 0.45) && (q[k][l] < 0.55))	accum (&qod_045_055, 1.0);
		else if ((q[k][l] > 0.55) && (q[k][l] < 0.65))	accum (&qod_055_065, 1.0);
		else if ((q[k][l] > 0.65) && (q[k][l] < 0.75))	accum (&qod_065_075, 1.0);
		else if ((q[k][l] > 0.75) && (q[k][l] < 0.85))	accum (&qod_075_085, 1.0);
		else if ((q[k][l] > 0.85) && (q[k][l] < 0.95))	accum (&qod_085_095, 1.0);
		else if ((q[k][l] > 0.95) && (q[k][l] < 1.0))	accum (&qod_095_100, 1.0);
	}
}
...
distribution_blocks_on_q()
{
	...
	accum (&Ave_ng_od,  accsum(&ng_od));
	accum (&Ave_VA_od,  accsum(&VA_od));
	accum (&Ave_VD_od,  accsum(&VD_od));

	accum (&Ave_qod_000_005,  accsum(&qod_000_005));
	accum (&Ave_qod_005_015,  accsum(&qod_005_015));
	accum (&Ave_qod_015_025,  accsum(&qod_015_025));
	accum (&Ave_qod_025_035,  accsum(&qod_025_035));
	accum (&Ave_qod_035_045,  accsum(&qod_035_045));
	accum (&Ave_qod_045_055,  accsum(&qod_045_055));
	accum (&Ave_qod_055_065,  accsum(&qod_055_065));
	accum (&Ave_qod_065_075,  accsum(&qod_065_075));
	accum (&Ave_qod_075_085,  accsum(&qod_075_085));
	accum (&Ave_qod_085_095,  accsum(&qod_085_095));
	accum (&Ave_qod_095_100,  accsum(&qod_095_100));
    ...
    if (accsum(&ng_od) != 0.0)
    {
      accum (&Ave_s_od,  accmean(&s_od));
      accum (&Ave_a_od,  accmean(&a_od));
      accum (&Ave_q_od,  accmean(&q_od));
      if (accmean(&s_od) != 0.0)   accum (&Ave_hs_od,  accmean(&hs_od));
      accum (&Ave_ha_od,  accmean(&ha_od));
    }
...
double q[][31], s[][31], hs[][31], a[][31], ha[][31], cumH[][31];
{
    ...
    mutsQN = mutationnumberQN();
    mutsOD = mutationnumberOD();

    if ( (muts + mutsL + mutsQN + mutsOD) == 0 ) goto label;
	...
	{
	    if (mutants_ocurred==(muts+mutsL+mutsQN+mutsOD))    goto label;
	    ...
	    cumH[NoSS_k[m]][NoSS_l[m]] = 0.0;
	 	...
	 	/* ****** Lethal, QN and OD mutations with effect on the trait ****** */
		 ...
		 if(mutants_ocurred <= mutsL)
		 {
			...
			ha[NoSS_k[m]][NoSS_l[m]] = 0.02;
		 }
		else if(mutants_ocurred <= mutsL+mutsQN)
		{
			s[NoSS_k[m]][NoSS_l[m]] = (-0.005);
			hs[NoSS_k[m]][NoSS_l[m]] = 0.5;
		}
		else if(mutants_ocurred <= (mutsL+mutsQN+mutsOD))
		{
			hs[NoSS_k[m]][NoSS_l[m]] = 99;
			if (uniform() < 0.5)	zs = sod1;
			else					zs = sod2;
			s[NoSS_k[m]][NoSS_l[m]] = zs;
			za = zs;
		 }
	    ...
	    /* ****** values of s, hs, a and ha for the new mutation ****** */
            if(whatproportion==0)
            {
	    		if(uniform() < Psi)
                {
                    ...
                    if (hs[NoSS_k[m]][NoSS_l[m]] == 99)
				{
					if (uniform()<0.5) s[NoSS_k[m]][NoSS_l[m]] = sod1;
					else			    s[NoSS_k[m]][NoSS_l[m]] = sod2;
                    }
                    if (hs[NoSS_k[m]][NoSS_l[m]] != 99)
                    {
                        ...
                        else
                        {
                           if (s[NoSS_k[m]][NoSS_l[m]] < 0.0)
                           {
                            ...
                            if (s[NoSS_k[m]][NoSS_l[m]] == -1.0)	hs[NoSS_k[m]][NoSS_l[m]]=0.02;
                        }
                    }
...
int mutationnumberQN ()
{
	int r;
	if ((2.0*(double)NIND*Lambda_QN < normalthreshold) &&
(exp(-2.0*(double)NIND*Lambda_QN) != 0.0) )
	{
		r= poisson(lastinmutantspoissontableQN, mutantspoissontableQN);
	}
	else r = (int)( normal(2.0*(double)NIND*Lambda_QN,
sqrt(2.0*(double)NIND*Lambda_QN)) );
	return(r);
}
...
printout()
{
    ...
    fprintf(fgen,"\n\n\n gen=%d  N_SO=%4.2f+-%4.2f     SK2=%4.2f+-%4.2f    N_Recom=%4.2f\n", gen+1, accmean(&num_SO[gen/block]), se(&num_SO[gen/block]), accmean(&SK2[gen/block]), se(&SK2[gen/block]), accmean(&numberofrecom[gen/block]));
    ...
    fprintf(fgen, "         addedfixed_s=%6.4f      Wmax=%6.4f\n", addedfixed_s, accmean(&Wmax[gen/block]));
	fprintf(fgen, "         L_s=%6.4f\n", (addedfixed_s - accmean(&gmean_s[gen/block])) / addedfixed_s);
    ...
    fprintf(fgen, "\n         SD   lost_sd=%4.2f   fix_sd=%4.2f   other_sd=%4.2f\n\n", 	lost_sd[gen/block]/(double)block,fix_sd[gen/block]/(double)block,other_sd[gen/block]/(double)block);
    ...
    else 	fprintf (fgen, "all values %6.2f+-%4.2f  %f+-%f  %f+-%f  %f+-%f  %f+-%f  %f+-%f\n\n", accmean(&Ave_ng_all), se(&Ave_ng_all), accmean(&Ave_s_all), se(&Ave_s_all), accmean(&Ave_a_all), se(&Ave_a_all), accmean(&Ave_hs_all), se(&Ave_hs_all), accmean(&Ave_ha_all), se(&Ave_ha_all), accmean(&Ave_q_all), se(&Ave_q_all));

    if(accsum(&Ave_ng_od) == 0.0)		/* do not print */;
    else 	fprintf (fgen, "OD %6.2f+-%4.2f  %f+-%f  %f+-%f  %f+-%f  %f+-%f  %f+-%f\n\n", accmean(&Ave_ng_od), se(&Ave_ng_od), accmean(&Ave_s_od), se(&Ave_s_od), accmean(&Ave_a_od), se(&Ave_a_od), accmean(&Ave_hs_od), se(&Ave_hs_od), accmean(&Ave_ha_od), se(&Ave_ha_od), accmean(&Ave_q_od), se(&Ave_q_od));

	if(accsum(&Ave_ng_od) == 0.0)		/* do not print */;
	else	fprintf(fgen, "%d    %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f   %6.4f\n",
		gen, accsum(&Ave_qod_000_005), accsum(&Ave_qod_005_015), accsum(&Ave_qod_015_025),
		accsum(&Ave_qod_025_035), accsum(&Ave_qod_035_045), accsum(&Ave_qod_045_055),
		accsum(&Ave_qod_055_065), accsum(&Ave_qod_065_075), accsum(&Ave_qod_075_085),
		accsum(&Ave_qod_085_095), accsum(&Ave_qod_095_100));
...
fprintf (fgen, "\n               n.genes              VA                           VD\n");
    ...
    if(accsum(&Ave_ng_od) == 0.0)		/* do not print */;
    else 	fprintf (fgen, "OD  %6.2f+-%4.2f           %f+-%f           %f+-%f\n\n", accmean(&Ave_ng_od), se(&Ave_ng_od), accmean(&Ave_VA_od), se(&Ave_VA_od), accmean(&Ave_VD_od), se(&Ave_VD_od));
...
settozero_gens_on_q()
{
	initacc(&ng_all); initacc(&s_all); initacc(&a_all); initacc(&hs_all); initacc(&ha_all); initacc(&q_all); initacc(&VA_all); initacc(&VD_all);
	initacc(&ng_od); initacc(&s_od); initacc(&a_od); initacc(&hs_od); initacc(&ha_od); initacc(&q_od); initacc(&VA_od); initacc(&VD_od);
	initacc(&qod_000_005); initacc(&qod_005_015); initacc(&qod_015_025); initacc(&qod_025_035); initacc(&qod_035_045);
	initacc(&qod_045_055); initacc(&qod_055_065); initacc(&qod_065_075); initacc(&qod_075_085); initacc(&qod_085_095); initacc(&qod_095_100);
...
settozero_blocks_on_q()
{
	initacc(&Ave_ng_all); initacc(&Ave_s_all); initacc(&Ave_a_all); initacc(&Ave_hs_all); initacc(&Ave_ha_all);  initacc(&Ave_q_all); initacc(&Ave_VA_all); initacc(&Ave_VD_all);
	initacc(&Ave_ng_od); initacc(&Ave_s_od); initacc(&Ave_a_od); initacc(&Ave_hs_od); initacc(&Ave_ha_od);  initacc(&Ave_q_od); initacc(&Ave_VA_od); initacc(&Ave_VD_od);
	initacc(&Ave_qod_000_005); initacc(&Ave_qod_005_015); initacc(&Ave_qod_015_025); initacc(&Ave_qod_025_035); initacc(&Ave_qod_035_045);
	initacc(&Ave_qod_045_055); initacc(&Ave_qod_055_065); initacc(&Ave_qod_065_075); initacc(&Ave_qod_075_085); initacc(&Ave_qod_085_095); initacc(&Ave_qod_095_100);
